...
users = config.fake_databasee['users']


# делаем проверку на админ ли боту пишет проверяем текст сообщения
@bot.message_handler(func=lambda call: True and call.from_user.id == config.tg_admin_id ) 
def all_users(call):
    count_users = len(users)
    text = 'g'
    count_in = 4
    all_lists  = math.ceil(count_users/count_in)
    if call.text == "Все юзеры":
        inline_markup = telebot.types.InlineKeyboardMarkup()
        for user in users[:4]:  # в цикле создаем 3 кнопки и добавляем их поочередно в нашу разметку 
            inline_markup.add(telebot.types.InlineKeyboardButton(text=f'Юзер: {user["name"]}',
                                                                callback_data=f"user_{user['id']}"))
        inline_markup.add(
            telebot.types.InlineKeyboardButton(text="Вперёд",  callback_data="menu"), 
            telebot.types.InlineKeyboardButton(text = f'{1}\{all_lists}',  callback_data='---'))
                # number_list = math.ceil((t+1)/count_in)
...
def get_other_list(call):
    text = 'g'
    count_users = len(users)
    all_lists  = math.ceil(count_users/4)
    if call.data.split('_')[0] == "Вторая часть":
        inline_markup = telebot.types.InlineKeyboardMarkup()
        for user in users[4:8]:  # в цикле создаем 3 кнопки и добавляем их поочередно в нашу разметку 
            inline_markup.add(telebot.types.InlineKeyboardButton(text=f'Юзер: {user["name"]}',
                                                                callback_data=f"user_{user['id']}"))
        inline_markup.add(
            telebot.types.InlineKeyboardButton(text="Вперёд",  callback_data='menu_2'),           
            telebot.types.InlineKeyboardButton(text = f'{all_lists}\{all_lists}',  callback_data='users'),
            telebot.types.InlineKeyboardButton(text="Назад",  callback_data='Все юзеры')
                        )
    elif call.data.split('_')[0] == "menu_2":
        inline_markup = telebot.types.InlineKeyboardMarkup()
        for user in users[8:]:  # в цикле создаем 3 кнопки и добавляем их поочередно в нашу разметку 
            inline_markup.add(telebot.types.InlineKeyboardButton(text=f'Юзер: {user["name"]}',
                                                                callback_data=f"user_{user['id']}"))
        inline_markup.add(
            telebot.types.InlineKeyboardButton(text="Вперёд",  callback_data='users_3'),           
            telebot.types.InlineKeyboardButton(text = f'{all}\{all_lists}',  callback_data='users'),
            telebot.types.InlineKeyboardButton(text="Назад",  callback_data='Все юзеры'))
