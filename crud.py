...
def create_wallet(user: pydantic_models.User = None, private_key: str = None, testnet: bool = False):
    ...
    if user:
        wallet = models.Wallet(user=user, private_key=raw_wallet.to_wif(), address=raw_wallet.address)
    else:
        wallet = models.Wallet(private_key=raw_wallet.to_wif(), address=raw_wallet.address)
    flush()
    return wallet
...
def create_user(tg_id: int, nick: str = None):
    if nick:
        user = models.User(tg_ID=tg_id, nick=nick, create_date=datetime.now(), wallet=create_wallet())
    else:
        user = models.User(tg_ID=tg_id, create_date=datetime.now(), wallet=create_wallet())  # сохраняем объект в базе данных, чтобы получить его айди
    return user
...
def create_transaction(sender: models.User, amount_btc_without_fee: float, receiver_address: str, fee: float | None = None, testnet: bool = False):
    ...
    if testnet:
        wallet_of_sender = bit.Key(sender.wallet.private_key)     # тут мы загружаем в переменную wallet_of_sender кошелек отправителя
    else:
        wallet_of_sender = bit.PrivateKeyTestnet(sender.wallet.private_key)    # и если мы в тестовой сети, то соответственно мы загружаем кошелек из тестовой сети
    ...
    if amount_btc_without_fee + fee >= sender.wallet.balance:  # если баланс кошелька меньше, полной стоимости транзакции, то возвращаем сообщение
        return f"Недостаточно средств на балансе!  \n Ваш баланс : {sender.wallet.balance}"
    # подготовливаем  кортеж в списке с данными для транзакции
    ...
    transaction = models.Transaction(sender=sender,
...
def update_all_wallets():
    ...
    for wallet in select(w for w in models.Wallet)[:]: 
...
def get_user_by_tg_id(tg_id: int):
    return models.User.select(lambda u: u.tg_ID == tg_id).first()


@db_session
def update_user(user: pydantic_models.User_to_update):
    user_to_update = models.User[user.id]
    if user.tg_ID:
